    ...
    return (
        <div className="max-w-7xl mx-auto animate-in fade-in duration-1000">
            <div className="mb-12">
                ...
                <p className="text-white/40 mt-3 text-xs uppercase tracking-[0.3em] font-bold">Extrato Financeiro B2B</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-8 relative overflow-hidden">
                    ...
                    <p className="text-[10px] uppercase tracking-[0.2em] text-white/40 font-bold mb-4">Total Acumulado</p>
                    <p className="text-4xl font-display text-white">
                        <span className="text-xl text-white/50 mr-2">R$</span>
                ...
                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-3xl p-8 relative overflow-hidden">
                    ...
                    <p className="text-[10px] uppercase tracking-[0.2em] text-emerald-500/70 font-bold mb-4">Liberado / Pago</p>
                ...
                <div className="bg-brand-gold/10 border border-brand-gold/30 rounded-3xl p-8 relative overflow-hidden shadow-[0_0_30px_rgba(197,160,89,0.1)]">
                    <div className="absolute top-0 right-0 p-8 opacity-20">
            ...
            <div className="bg-white/[0.01] border border-white/5 rounded-[3rem] p-10">
                <div className="flex justify-between items-center mb-10 pb-6 border-b border-white/5">
                    <h3 className="text-sm font-bold uppercase tracking-[0.2em] text-white/60">Extrato Detalhado</h3>
                    <div className="flex gap-4">
                        <button className="px-6 py-2 rounded-xl bg-white/5 text-[10px] uppercase tracking-widest font-bold text-white hover:bg-white/10 transition-colors">Meses Anteriores</button>
                    </div>
                </div>
                    ...
                    commissions.map((comm) => (
                        <div key={comm.id} className="group flex flex-col md:flex-row md:items-center justify-between p-6 bg-white/[0.02] hover:bg-white/[0.04] border border-white/5 rounded-2xl transition-all duration-300">
                            ...
                            <div className="flex items-center gap-6 mb-4 md:mb-0">
                                <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${comm.status === 'paid' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-brand-gold/10 text-brand-gold'}`}>
                                    {comm.status === 'paid' ? <CheckCircle2 className="w-5 h-5" /> : <Clock className="w-5 h-5" />}
                                </div>

                                <div>
                                    <p className="text-sm font-bold text-white group-hover:text-brand-gold transition-colors">{comm.client_name}</p>
                            ...
                            <div className="flex items-end md:items-center gap-8 md:gap-12 flex-row-reverse md:flex-row">
                                <div className="text-right">
                                    <p className={`text-[10px] uppercase tracking-[0.2em] font-bold mb-1 ${comm.status === 'paid' ? 'text-emerald-500/70' : 'text-brand-gold/70'}`}>
                                ...
                                <div className="text-right">
                                    <p className={`text-xl font-display ${comm.status === 'paid' ? 'text-emerald-400' : 'text-brand-gold'}`}>
                                        + R$ {comm.amount.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                                    </p>
                                    <p className="text-[9px] uppercase tracking-widest text-white/30 font-bold mt-1">10% Cms</p>
                                </div>
                            </div>
